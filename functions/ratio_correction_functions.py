...
def compare_correction_figure(com_bl,ratio_bl, correction_x_bl, avg_ratio_bl, std_ratio_bl, correction_x_bl_2, avg_ratio_bl_2, std_ratio_bl_2,region, height, width):
    ...
    ax.set_ylabel('Occurrence frequency correcrtion factor')
...
def draw_correction_figures(com_lobe, ratio_lobe, correction_x_lobe,avg_ratio_lobe,  std_ratio_lobe, com_bl,ratio_bl, correction_x_bl, avg_ratio_bl, std_ratio_bl, com_ps,ratio_ps,correction_x_ps, avg_ratio_ps, std_ratio_ps ):
...
#    axs[1].errorbar(correction_x_bl_2, avg_ratio_bl_2, std_ratio_bl_2, marker='s', color='red', mfc='black',mec='black', ms=3, mew=1, label='Median Value')
    ...
    axs[0].set_xlabel('Occurrence frequency with compression')
    axs[1].set_xlabel('Occurrence frequency with compression')
    axs[2].set_xlabel('Occurrence frequency with compression')

    axs[0].set_ylabel('Occurrence frequency correcrtion factor')
    axs[1].set_ylabel('Occurrence frequency correcrtion factor')
    axs[2].set_ylabel('Occurrence frequency correcrtion factor')

    plt.savefig('figure4.svg', format='svg', dpi=300)
    plt.savefig('figure4.png', format='png', dpi=300)
    
    return 1

def read_data(path1, path2, grid_str, region, direction_str, start_time, end_time):
    file_1 = path1 + "/" + grid_str + "/" + region + "_" + direction_str + "/events/2d/" + region + "_" + direction_str + "__ratio_" + start_time + "_to_" + end_time + "_X_GSM_vs_Y_GSM.csv"

    data_1 = np.array(pd.read_csv(file_1))
    data_1 = data_1[:,1:21]
    data_1 =  data_1.reshape((400))  

    file_2 = path2+"/"+grid_str+"/"+region+"_"+direction_str+"/events/2d/"+region+"_"+direction_str + "__ratio_" + start_time+"_to_"+end_time+"_X_GSM_vs_Y_GSM.csv"
...
def calculate_avg(x,y, x_low, x_high, ):
    ...
    for i in range(0, x_low.shape[0]):
        ...
        if y[index].shape[0] > 0:
            y_avg[i] = statistics.median(y[index])
